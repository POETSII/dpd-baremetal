...
int main(int argc, char *argv[]) {
    ...
    unsigned cells_per_dimension_with_walls = N + 1;
    ...
    for(int i=0; i < total_beads; i++) {
        ...
        while(!added) {
            ...
            b1->id = b_uid++;
            // b1->type = 0;
            b1->pos.set((rand() / (float)RAND_MAX * problem_size) + 0.5, (rand() / (float)RAND_MAX * problem_size) + 0.5, (rand() / (float)RAND_MAX * problem_size) + 0.5);
